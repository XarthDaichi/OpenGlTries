...
float ToRadianf(float x) {
    float pi = 3.14159265359f;
    return (x * (pi / 180.0f));
}

float ToDegreef(float x) {
    float pi = 3.14159265359f;
    return (x * (180.f / pi));
}
...
void Matrix4f::InitRotateTransform(float RotateX, float RotateY, float RotateZ)
{
    ...
    float x = ToRadianf(RotateX);
    float y = ToRadianf(RotateY);
    float z = ToRadianf(RotateZ);
...
void Matrix4f::InitPersProjTransform(const PersProjInfo& p)
{
    ...
    float tanHalfFOV = tanf(ToRadianf(p.FOV / 2.0f));
...
{
    float HalfAngleInRadians = ToRadianf(Angle/2);
